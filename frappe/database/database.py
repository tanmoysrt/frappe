...
from frappe.dispatch import on_loop_thread
...
class Database:
	...
	def sql(
		...
		*,
		...
		"""
		if on_loop_thread():
			# Phase 8 fail-fast: a sync DB call here blocks the WHOLE process
			# (single event loop), not one worker like under gunicorn
			raise RuntimeError(
				"sync frappe.db call on the event loop thread — "
				"use `await frappe.db.aio.<method>(...)` in async handlers instead"
			)
